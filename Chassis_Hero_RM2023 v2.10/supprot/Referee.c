...
*/
uint8_t UI_DMA_watch1=5;//定义个变量看看发送UI的DMA有没有发出去
uint8_t UI_DMA_watch2=5;//定义个变量看看发送UI的DMA有没有发出去
uint8_t UI_DMA_watch3=5;//定义个变量看看发送UI的DMA有没有发出去
uint8_t UI_DMA_watch4=5;//定义个变量看看发送UI的DMA有没有发出去
void UI_PushUp_Graphs(uint8_t Counter /* 1,2,5,7 */, void *Graphs /* 与Counter相一致的UI_Graphx结构体头指针 */, uint8_t RobotID)
{
	...
	/* 使用串口PushUp到裁判系统 */
	     if(Counter == 1) UI_DMA_watch1=HAL_UART_Transmit_DMA(&Referee_UART, (uint8_t *)Graph, sizeof(UI_Graph1_t));
	else if(Counter == 2) UI_DMA_watch2=HAL_UART_Transmit_DMA(&Referee_UART, (uint8_t *)Graph, sizeof(UI_Graph2_t));
	else if(Counter == 5) UI_DMA_watch3=HAL_UART_Transmit_DMA(&Referee_UART, (uint8_t *)Graph, sizeof(UI_Graph5_t));
	else if(Counter == 7) UI_DMA_watch4=HAL_UART_Transmit_DMA(&Referee_UART, (uint8_t *)Graph, sizeof(UI_Graph7_t));
}
